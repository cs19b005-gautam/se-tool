import os
import sys
import argparse
import gensim
import tempfile
import json
from enum import Enum
from nltk.tokenize import word_tokenize
from collections import OrderedDict

sum = 0
count = 0
percentage = 0
clonePairString = ''

# TODO remove unnecessary file_extensions
src_code_file_extensions = ["h", "c", "cpp", "cc", "java", "py", "cs"]
file_column_label = "File"

# shows the percentage of similarity between 2 files
similar_column_label = "Similarity (%)"
similar_label_length = len(similar_column_label)


# A ReturnCode compared can either be success or bad_input or can be theshold_exceed
class ReturnCode(Enum):
    SUCCESS = 0
    BAD_INPUT = 1
    THRESHOLD_EXCEEDED = 2



# TODO command-line-interface colors are not needed and can be removed later after code fixes
class CliColors:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKGREEN = '\033[92m'
    WARNING = '\033[93m'
    FAIL = '\033[91m'
    ENDC = '\033[0m'
    BOLD = '\033[1m'
    UNDERLINE = '\033[4m'


def get_all_src_code_from_directory(directory, file_extensions):
    
    # Getting a list with each each element as all the source-code-files within the directory
    
    src_code_files = list()
    for dirpath, _, filenames in os.walk(directory):

        for name in filenames:

            _, file_extension = os.path.splitext(name)
            if file_extension[1:] in file_extensions:

                filename = os.path.join(dirpath, name)
                src_code_files.append(filename)

    return src_code_files


# printing based on the text being machine_readable
def conditional_print(text, machine_readable_output):
    if not machine_readable_output:
        print(text)


def main():
    parser_description = CliColors.HEADER + CliColors.BOLD + \
        "=== Duplicate Code Detection Tool ===" + CliColors.ENDC
    
    parser = argparse.ArgumentParser(description=parser_description)
    
    parser.add_argument("-t", "--fail-threshold", type=int, default=100, help="The maximum allowed similarity before the script exits with an error.")
    


    group = parser.add_mutually_exclusive_group(required=True)
    
    group.add_argument("-d", "--directories", nargs="+", help="Check for similarities between all files of the specified directories.")
    group.add_argument('-f', "--files", nargs="+", help="Check for similarities between specified files. \
                        The more files are supplied the more accurate are the results.")
    
    
    parser.add_argument("--ignore-directories", nargs="+", default=list(),help="Directories to ignore.")
    parser.add_argument("--ignore-files", nargs="+", help="Files to ignore.")
    parser.add_argument("-j", "--json", type=bool, default=False, help="Print output as JSON.")
    parser.add_argument("--project-root-dir", type=str, default=str(), help="The relative path to the project root directory to be removed when printing out results.")
    parser.add_argument("--file-extensions", nargs="+", default=src_code_file_extensions, help="File extensions to check for similarities.")
    parser.add_argument("--ignore-threshold", type=int, default=0, help="Don't print out similarity below the ignore threshold")
    
    
    args = parser.parse_args()

    result = get_similarities(args.fail_threshold, args.directories, args.files, args.ignore_directories,
                 args.ignore_files, args.json, args.project_root_dir, args.file_extensions,
                 args.ignore_threshold)


    return result





def get_similarities(fail_threshold, directories, files, ignore_directories, ignore_files,
        json_output, project_root_dir, file_extensions, ignore_threshold):

    
    # Determine which files to compare for similarities
    src_code_files = list()
    files_to_ignore = list()


    if directories:
        for directory in directories:
            if not os.path.isdir(directory):
                print("Path does not exist or is not a directory:", directory)
                return (ReturnCode.BAD_INPUT, {})
            src_code_files += get_all_src_code_from_directory(directory, file_extensions)
        for directory in ignore_directories:
            files_to_ignore += get_all_src_code_from_directory(
                directory, file_extensions)
    else:
        if len(files) < 2:
            print("Too few files to compare, you need to supply at least 2")
            return (ReturnCode.BAD_INPUT, {})
        for supplied_file in files:
            if not os.path.isfile(supplied_file):
                print("Supplied file does not exist:", supplied_file)
                return (ReturnCode.BAD_INPUT, {})
        src_code_files = files

    files_to_ignore += ignore_files if ignore_files else list()
    src_code_files = list(set(src_code_files) - set(files_to_ignore))
    if len(src_code_files) < 2:
        print("Not enough source code files found")
        return (ReturnCode.BAD_INPUT, {})
    src_code_files = [os.path.abspath(f) for f in src_code_files]

    # Get the absolute project root directory path to remove when printing out the results
    if project_root_dir:
        if not os.path.isdir(project_root_dir):
            print("The project root directory does not exist or is not a directory:", project_root_dir)
            return (ReturnCode.BAD_INPUT, {})
        project_root_dir = os.path.abspath(project_root_dir)
        project_root_dir = os.path.join(
            project_root_dir, '')  # Add the trailing slash

    # Find the largest string length to format the textual output
    largest_string_length = len(max(src_code_files, key=len).replace(project_root_dir, ""))

    # Parse the contents of all the source files
    src_code = OrderedDict()
    for src_code_file in src_code_files:
        with open(src_code_file, 'r') as f:
            # Store source code with the file path as the key
            src_code[src_code_file] = f.read()

    # Create a Similarity object of all the source code
    gen_docs = [[word.lower() for word in word_tokenize(src_code[source_file])]
                for source_file in src_code]
    dictionary = gensim.corpora.Dictionary(gen_docs)
    corpus = [dictionary.doc2bow(gen_doc) for gen_doc in gen_docs]
    tf_idf = gensim.models.TfidfModel(corpus)
    sims = gensim.similarities.Similarity(tempfile.gettempdir() + os.sep, tf_idf[corpus], num_features=len(dictionary))

    exit_code = ReturnCode.SUCCESS
    code_similarity = dict()
    for source_file in src_code:
        # Check for similarities
        query_doc = [w.lower()
                     for w in word_tokenize(src_code[source_file])]
        query_doc_bow = dictionary.doc2bow(query_doc)
        query_doc_tf_idf = tf_idf[query_doc_bow]

        short_source_file_path = source_file.replace(project_root_dir, '')
        conditional_print("\n\n\n" + CliColors.HEADER +
                          "Code duplication probability for " + short_source_file_path + CliColors.ENDC, json_output)
        
        conditional_print("-" * (largest_string_length +
                                 similar_label_length), json_output)
        
        conditional_print(CliColors.BOLD + "%s %s" %
                          (file_column_label.center(largest_string_length), similar_column_label) + CliColors.ENDC, json_output)
        
        conditional_print("-" * (largest_string_length +
                                 similar_label_length), json_output)

        code_similarity[short_source_file_path] = dict()

        clonePairString = clonePairString+short_source_file_path
        for similarity, source in zip(sims[query_doc_tf_idf], src_code):
            # Ignore similarities for the same file
            if source == source_file:
                continue
            similarity_percentage = similarity * 100
            sum = sum + similarity_percentage
            count = count + 1
            # Ignore very low similarity
            if similarity_percentage < ignore_threshold:
                continue
            short_source_path = source.replace(project_root_dir, '')
            code_similarity[short_source_file_path][short_source_path] = round(
                similarity_percentage, 2)
            if similarity_percentage > fail_threshold:
                exit_code = ReturnCode.THRESHOLD_EXCEEDED
            color = CliColors.OKGREEN if similarity_percentage < 10 else (
                CliColors.WARNING if similarity_percentage < 20 else CliColors.FAIL)
            if color == CliColors.OKGREEN:
                continue
            else:
                conditional_print("%s     " % (short_source_path.ljust(largest_string_length)) +
                              color + "%.2f" % (similarity_percentage) + CliColors.ENDC, json_output)
            clonePairString = clonePairString+">"+short_source_path+">"+similarity_percentage
        clonePairString = clonePairString+"&"
    
    if exit_code == ReturnCode.THRESHOLD_EXCEEDED:
        conditional_print("Code duplication threshold exceeded. Please consult logs.", json_output)
    
    if json_output:
        similarities_json = json.dumps(code_similarity, indent=4)
        print(similarities_json)

    return (exit_code, code_similarity)




if __name__ == "__main__":
    exit_code, _ = main()
    percentage = sum/count
    file1 = open('percentage.txt','w')
    file1.write(percentage)
    file1.close()
    file2 = open('clone_pair_string.txt','w')
    file2.write(clonePairString)
    file2.close()
    sys.exit(exit_code.value)
